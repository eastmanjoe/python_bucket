...
if __name__ == "__main__":
    ...
    try:
        ...
        # Serial Number
        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 0)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 1)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 3)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 7)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 135)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 134)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 132)
        print regs

        sleep(1)

        regs = master.execute(int(args.modbus_address), cst.WRITE_MULTIPLE_COILS, 7, 128)
        print regs
